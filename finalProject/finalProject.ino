 ...
 * it also receives inputs from processing and controls RGB light and speaker accordingly.
 */
...
void setup() {
  ...
  // Starting the conversation here since both Arduino and Processing wait for each other before they send anything
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (Serial.read() == '\n') {
      ...
      // Sending the data for potentiometer input to Processing
      Serial.println(String(analogRead(potentiometer)) + "," + color);
    }
  }

  // If a coin has been eaten
  if (coinEaten == 1)
  {
    coinEaten = 0;
